...
public abstract class AbstractChildInstantiatingAndLayoutControllableUIElement extends AbstractChildrenContainingUIElement implements ITemplateInstanceDataProvidingElement {
      ...
      @NotNull final IDependencyObject<Orientation> orientation)
    {
        ...
        this.templateConstructionData = DependencyObjectHelper.createFromValue(IProxy.getInstance().getBuilderManager().getConstructionData().build());
        this.source = source;
    ...
    {
        final Wrapper wrappingRegion = new Wrapper(
