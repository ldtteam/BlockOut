...
public class TextField extends AbstractSimpleUIElement implements IDrawableUIElement, IClientSideClickAcceptingUIElement, IClientSideKeyAcceptingUIElement
{
    ...
    private void writeText(final String str)
    {
        ...
        IProxy.getInstance().getNetworkingManager().sendToServer(new TextFieldUpdateContentsMessage(getId(), resultBuffer.toString()));

        int newCursorPos = selectionEnd + (insertAt - selectionEnd) + insertedLength;

        IProxy.getInstance().getNetworkingManager().sendToServer(new TextFieldUpdateSelectionEndOrCursorPositionMessage(getId(), newCursorPos, newCursorPos));
    }
    ...
    private void deleteFromCursor(final int count)
    {
        ...
        else
        {
            ...
            IProxy.getInstance().getNetworkingManager().sendToServer(new TextFieldUpdateContentsMessage(getId(), result));
            if (backwards)
            {
                IProxy.getInstance().getNetworkingManager().sendToServer(new TextFieldUpdateSelectionEndOrCursorPositionMessage(getId(), selectionEnd + count, selectionEnd+count));
            }
        }
    }
    ...
    private void handleKey(final char c, final KeyboardKey key)
    {
        switch (key)
        {
            ...
            case KEY_KP_ENTER:
            case KEY_ENTER:
                IProxy.getInstance().getNetworkingManager().sendToServer(new TextFieldOnEnterPressed(getId()));
                break;
            case KEY_KP_0:
            case KEY_KP_1:
            case KEY_KP_2:
    ...
    private void handleTab()
    {
        ...
        optionalNextElement
          ...
          .ifPresent(element -> {
              IProxy.getInstance().getNetworkingManager().sendToServer(new TextFieldTabPressedMessage(element.getId()));
          });
    }
    ...
    private void handleArrowKeys(final KeyboardKey key)
    {
        ...
        if (Screen.hasShiftDown())
        {
            ...
            {
                IProxy.getInstance().getNetworkingManager().sendToServer(new TextFieldUpdateSelectionEndOrCursorPositionMessage(getId(), getNthWordFromCursor(direction), getSelectionEnd()));
            }
            else
            {
                IProxy.getInstance().getNetworkingManager().sendToServer(new TextFieldUpdateSelectionEndOrCursorPositionMessage(getId(), cursorPosition + direction, getSelectionEnd()));
            }
        }
        else if (Screen.hasControlDown())
        {
            IProxy.getInstance().getNetworkingManager().sendToServer(new TextFieldUpdateSelectionEndOrCursorPositionMessage(getId(), getNthWordFromCursor(direction), getNthWordFromCursor(direction)));
        }
        else
        {
            IProxy.getInstance().getNetworkingManager().sendToServer(new TextFieldUpdateSelectionEndOrCursorPositionMessage(getId(), cursorPosition + direction, cursorPosition + direction));
        }
    }
    ...
    private void handleHomeEnd(final KeyboardKey key)
    {
        ...
        if (Screen.hasShiftDown())
        {
            IProxy.getInstance().getNetworkingManager().sendToServer(new TextFieldUpdateSelectionEndOrCursorPositionMessage(getId(), position, getSelectionEnd()));
        }
        else
        {
            IProxy.getInstance().getNetworkingManager().sendToServer(new TextFieldUpdateSelectionEndOrCursorPositionMessage(getId(), position, position));
        }
    }
    ...
    public boolean onMouseClickBeginClient(final int localX, final int localY, final MouseButton button)
    {
        ...
        IProxy.getInstance().getNetworkingManager().sendToServer(new TextFieldUpdateSelectionEndOrCursorPositionMessage(getId(), trimmedString.length() + scrollOffset, selectionEnd));
    ...
    public boolean onMouseClickMoveClient(final int localX, final int localY, final MouseButton button, final float timeElapsed)
    {
        ...
        IProxy.getInstance().getNetworkingManager().sendToServer(new TextFieldUpdateSelectionEndOrCursorPositionMessage(getId(), cursorPosition, trimmedString.length() + scrollOffset));
    ...
    public boolean onKeyPressedClient(final int character, final KeyboardKey key)
    {
        ...
        {
            case 1:
                IProxy.getInstance().getNetworkingManager().sendToServer(new TextFieldUpdateSelectionEndOrCursorPositionMessage(getId(), getContents().length(), 0));
                break;
            case 3:
